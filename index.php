...
if (isset($_GET["registered"]) && $_GET["registered"] === "1") {?>
            <div style="margin: 20px; margin-top: 20px;" class="alert alert-success alert-dismissible fade show" role="alert">
                    Inscription réussie ! Vous pouvez maintenant vous <a class="alert-link" href="php/login.php">connecter</a>.                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                </div>
<?php
};
...
if ($page === "cgu") {
    require_once __DIR__ . "/MVC/Model/php/CGUmodel.php";
    require_once __DIR__ . "/MVC/Controller/php/CGUcontroller.php";
    $model = new CGUModel();
...
if ($page === "faq") {
    require_once __DIR__ . "/MVC/Model/php/FAQmodel.php";
    require_once __DIR__ . "/MVC/Controller/php/FAQcontroller.php";
    $model = new FAQModel();
...
if ($page === "login") {
    require_once __DIR__ . "/MVC/View/php/Loginview.php";
    $view = new LoginView();
    $view->render();
    exit();
}
...
if ($page === "messagerie" || $page === "listemessagerie") {
    ...
    require_once __DIR__ . "/MVC/Model/php/Messageriemodel.php";
    require_once __DIR__ . "/MVC/Controller/php/Messageriecontroller.php";
...
if ($page === "profil") {
    require_once __DIR__ . "/php/db.php";
    $userId = $_SESSION["user_id"] ?? 1;
    require_once __DIR__ . "/MVC/Model/php/Profilmodel.php";
    require_once __DIR__ . "/MVC/Controller/php/Profilcontroller.php";
    $model = new Profilmodel($pdo);
    $controller = new Profilcontroller($model);
    $profile = $controller->viewProfile($userId);
    require_once __DIR__ . "/MVC/View/php/Profilview.php";
    $view = new Profilview();
...
<html lang="fr">
  ...
  <head>
    ...
    <?php require "php/db2withoutlogin.php"; ?>

    <?php
      // Charger logements pour le carrousel — ne garder que ceux qui ont une photo
      require_once __DIR__ . '/php/models/LogementModel.php';
   ...
   <header class="topbar">
    <a href="php/index.php" class="topbar-logo">
  ...
  <div class="burger-menu">
    ...
    <nav class="topbar-nav">
      <a class="nav-link active-link" href="php/index">Accueil</a>
      <a class="nav-link" href="php/logements">Recherche</a>
      <?php if (!$isEtudiant): ?>
      <a class="nav-link" href="php/publish.php">Publier une annonce</a>
      <?php endif; ?>
      <?php if (!$isEtudiant): ?>
      <a class="nav-link" href="php/logements?view=mesannonces">Mes annonces</a>               
      <?php endif; ?>
      <?php if ($isEtudiant): ?>
      <a class="nav-link" href="php/candidatures.php">Mes candidatures</a>        
      <?php endif; ?>      
      <a class="nav-link" href="php/listemessagerie.php">Ma messagerie</a>
      <?php if ($isAdmin): ?> 
          <a class="nav-link" href="php/admin.php">Admin ⚙️</a>
      <?php endif; ?>

      <a class="nav-link " href="php/profil.php">Mon profil</a>   
    </nav>
  </header>
  ...
  <body>
    ...
    <div>
      <section class="hero text-center text-white">
        ...
        <form action="php/logements.php" method="GET" class="d-flex justify-content-center align-items-center mb-5">
          <input
            type="text"
            name="search"
            class=" search-input"
            ...
            <div class="carousel-inner">
              <?php if (!empty($carouselLogements)): ?>
                <?php foreach ($carouselLogements as $i => $lg): ?>
                  <div class="carousel-item <?php echo $i === 0 ? 'active' : ''; ?>">
                    <a href="php/logement.php?id=<?php echo intval($lg['ID']); ?>" title="<?php echo htmlspecialchars($lg['titre'] ?: 'Voir le logement'); ?>" style="display:block; color:inherit; text-decoration:none; cursor:pointer;">
                      <img src="<?php echo htmlspecialchars(substr($lg['photo_url'], 3) ?: 'https://via.placeholder.com/1000x500?text=No+Photo'); ?>" class="d-block w-100" alt="<?php echo htmlspecialchars($lg['titre'] ?: 'Logement'); ?>" />
              ...
              <?php else: ?>
                ...
                <div class="carousel-item active">
                  <a href="php/logements.php" style="display:block; color:inherit; text-decoration:none; cursor:pointer;">
            ...
            <?php
              ...
              if ($latestResult && $latestResult->num_rows > 0) {
                while ($row = $latestResult->fetch_assoc()) {
                  ...
                  $photoUrl = substr($photoUrl, 3);
                  ?>
                  <div class="col-md-4">
                    <a href="php/logement.php?id=<?php echo intval($row['ID']); ?>" class="logement-link">
